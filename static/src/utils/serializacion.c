...
pcb_t* deserializar_proceso(int cliente_socket){
	...
	int bytes_recibidos = recv(cliente_socket, &pcb_recibido->pid, sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Proceso: %d", pcb_recibido->pid); 
	format_trace_log("serializacion@deserializar_proceso", "Bytes: %d", bytes_recibidos); 

	recv(cliente_socket, &(pcb_recibido->tamanio), sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Tamaño del proceso: %d", pcb_recibido->tamanio); 

	recv(cliente_socket, &(pcb_recibido->program_counter), sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Program Counter: %d", pcb_recibido->program_counter); 

	recv(cliente_socket, &pcb_recibido->tabla_paginas, sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Nro Tabla de Paginas: %d", pcb_recibido->tabla_paginas); 

	recv(cliente_socket, &pcb_recibido->estado, sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Estado: %d", pcb_recibido->estado); 	

	recv(cliente_socket, &pcb_recibido->estimacion_rafaga, sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Estimacion Rafaga: %d", pcb_recibido->estimacion_rafaga); 

	recv(cliente_socket, &pcb_recibido->estimacion, sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Estimacion: %d", pcb_recibido->estimacion); 

	recv(cliente_socket, &pcb_recibido->duracion_ultima_rafaga, sizeof(uint32_t), false);
	format_trace_log("serializacion@deserializar_proceso", "Duracion ultima rafaga: %d", pcb_recibido->duracion_ultima_rafaga); 

	//recv(cliente_socket, &pcb_recibido->rafaga_actual, sizeof(rango_tiempo_t), false);
	//format_trace_log("serializacion@deserializar_proceso", "Rafaga actual: %d", pcb_recibido->rafaga_actual); 

    pcb_recibido->lista_instrucciones = recibir_lista_de_instrucciones(cliente_socket);
